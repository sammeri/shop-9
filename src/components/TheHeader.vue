...
// computed
const favoritesCounter = computed(() => favoritesStore.counter);
const cartCounter = computed(() => cartStore.counter);
</script>

<template>
  <header
    class="relative flex flex-col items-center justify-center bg-linear-to-r from-stone-400 to-gray-500 px-4 py-2 font-[PT_Sans,sans-serif] shadow-xl/30 shadow-amber-700"
  >
    <div class="container flex w-full items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <router-link to="/favorites" class="flex gap-1 font-normal text-gray-200">
          <div class="relative flex flex-col items-center">
            <vue-feather class="cursor-pointer" stroke="#e5e7eb" type="heart"></vue-feather>
            <span class="relative text-sm">Избранное</span>
            <!-- v-if="favoritesCounter" -->

            <span
              class="absolute -top-2 left-7/12 rounded-full border border-red-600 bg-red-600 px-1.25 text-xs opacity-0 duration-400"
              :class="{ 'opacity-100': favoritesCounter }"
            >
              {{ favoritesCounter }}
            </span>
          </div>
        </router-link>
        <router-link class="flex gap-1 font-normal text-gray-200" to="/cart">
          <div class="relative flex flex-col items-center">
            <vue-feather class="cursor-pointer" stroke="#e5e7eb" type="shopping-bag"></vue-feather>
            <span class="text-sm">Корзина</span>
            <span
              class="absolute -top-2 left-7/12 rounded-full border border-red-600 bg-red-600 px-1.25 text-xs opacity-0 duration-400"
              :class="{ 'opacity-100': cartCounter }"
            >
              {{ cartCounter }}
            </span>
          </div>
